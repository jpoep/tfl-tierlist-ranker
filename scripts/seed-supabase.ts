...
const checkExisting = Effect.tryPromise({
	try: () => supabase.from("pokemon").select("id", { count: "exact", head: true }),
	catch: (cause) => new Error(`Failed to check existing pokemon: ${String(cause)}`),
}).pipe(
	Effect.flatMap(({ count, error }) => {
		if (error) return Effect.fail(new Error(`Supabase error: ${error.message}`));
...
const seedPokemon = (asset: PokemonAsset) =>
	Effect.gen(function* () {
		yield* Console.log(`Seeding ${asset.pokemon.length} Pokémon in batches of ${BATCH_SIZE}…`);
		...
		for (let i = 0; i < asset.pokemon.length; i += BATCH_SIZE) {
			...
			const { error } = yield* Effect.tryPromise({
				try: () => supabase.from("pokemon").upsert(rows, { onConflict: "id" }),
				catch: (cause) =>
					new Error(`Failed to upsert pokemon batch ${i}–${i + batch.length}: ${String(cause)}`),
			});

			if (error) {
				return yield* Effect.fail(new Error(`Supabase upsert error: ${error.message}`));
			}

			inserted += batch.length;
			yield* Console.log(`  ✓ ${inserted}/${asset.pokemon.length} Pokémon upserted`);
		}
...
const seedRatings = (asset: PokemonAsset) =>
	Effect.gen(function* () {
		...
		for (let i = 0; i < asset.pokemon.length; i += BATCH_SIZE) {
			...
			const { error } = yield* Effect.tryPromise({
				try: () =>
					...
					new Error(`Failed to upsert ratings batch ${i}–${i + batch.length}: ${String(cause)}`),
			});
		...
		yield* Console.log(`  ✓ ${inserted} initial ratings upserted (skipped any existing)`);
		return inserted;
	});
