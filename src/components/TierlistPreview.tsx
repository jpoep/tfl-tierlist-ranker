...
const TierRow = ({ tier, assignments, compact }: TierRowProps) => {
  ...
  return (
    <div
      className={`flex min-h-12 gap-0 overflow-hidden rounded-lg border ${meta.border}`}
    >
      {/* Tier label column */}
      <div
      ...
      >
        <span
          className={`font-black ${meta.text} ${compact ? "text-sm" : "text-lg"}`}
        >
          {meta.label}
        </span>
      </div>
  ...
  return (
    <div className="flex w-full flex-col gap-3">
      ...
      <div className="flex items-center justify-between gap-2">
        <h3
          className={`font-semibold text-white/80 ${compact ? "text-sm" : "text-base"}`}
        >
          Aktuelle Tierlist
        </h3>
