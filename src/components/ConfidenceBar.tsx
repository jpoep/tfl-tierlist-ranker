...
const confidenceLabel = (score: number): { label: string; colour: string } => {
	if (score >= 0.9) return { label: "Tierlist ist würdig", colour: "text-emerald-400" };
	if (score >= 0.7) return { label: "Hohe Confidence", colour: "text-lime-400" };
	if (score >= 0.5) return { label: "Mittlere Confidence", colour: "text-yellow-400" };
	if (score >= 0.25) return { label: "Wenig Confidence", colour: "text-orange-400" };
			...
			<div className="mb-3 flex items-baseline justify-between gap-2">
				<div className="flex items-baseline gap-2">
					...
					<span className="text-xs text-white/30">
						{totalVotes.toLocaleString()} Stimme{totalVotes !== 1 ? "n" : ""} abgegeben
					</span>
				</div>
