 ...
 */
const uncertaintyScore = (r: Rating): number =>
  Math.max(0, Math.min(1, r.sigma / INITIAL_SIGMA));
...
export const selectNextPair = (
  ...
  Effect.gen(function* () {
    ...
    while (
      candidates.length < CANDIDATE_SAMPLE_SIZE &&
      ...
      const uncScore =
        (uncertaintyScore(left.rating) + uncertaintyScore(right.rating)) / 2;

      // Proximity component: how similar their ordinals are.
      // Distance of 0 → proximity of 1 (most informative).
      const ordinalDist =
        Math.abs(left.rating.ordinal - right.rating.ordinal) / safeRange;
