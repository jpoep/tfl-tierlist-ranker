...
export const stdDevBandTiers: TierStrategy = (sorted) => {
  ...
  const variance =
    ordinals.reduce((acc, o) => acc + (o - mean) ** 2, 0) / ordinals.length;
...
export const kmeansTiers: TierStrategy = (sorted) => {
  ...
  const clusterToRank = new Map(
    clusterRank.map(({ clusterIdx, rank }) => [clusterIdx, rank]),
  );
...
export const computeTierlist = (
  ...
  const sorted = [...ratedPokemon].sort(
    (a, b) => b.rating.ordinal - a.rating.ordinal,
  );
