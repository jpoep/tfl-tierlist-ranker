...
export const useTierlist = (
	...
	useEffect(() => {
		...
		const fetchInitialData = async () => {
			const [ratingsResult, countResult] = await Promise.all([
				...
				supabase.from("matchups").select("*", { count: "exact", head: true }).eq("skipped", false),
			]);

			if (cancelled) return;

			if (ratingsResult.error) {
				console.error("[useTierlist] failed to fetch ratings:", ratingsResult.error);
				return;
			}
	...
	const result = useMemo<TierlistResult | undefined>(() => {
		...
		if (state.pokemon.length === 0 || state.ratings.length === 0) return undefined;

		const ratingMap = new Map<number, RatingRow>(state.ratings.map((r) => [r.pokemon_id, r]));
